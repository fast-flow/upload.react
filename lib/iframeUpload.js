...
let createDom = function(pcls,uid,containerId) {
    ...
    var container = document.getElementById(pcls+"-iframe-label_"+containerId);
...
class IframeUpload extends Component {
    ...
    componentWillMount = () => {
        ...
        /* ie9- 不支持https:// */
        try {
            let actionErrorMsg = /^https/.test(self.props.action) && navigator.userAgent.toLowerCase().match(/msie ([\d.]+)/)[1] < 9 ;
            if(actionErrorMsg){
                alert('ie8及ie8- 不支持https') ;
            };
        } catch (e) {
            console.log(e) ;
        }
        ...
        return (
            <span className={`${pcls}-iframe`} >
                <label  className={`${pcls}-iframe-label`}
                        id={`${pcls}-iframe-label_${self.state.uid}`}
                        onClick={function(e){
                            e.preventDefault();
                            /* STEP :
                                1. 生成uid
                            ...
                            */
                            let uid = getUid()
                            createDom(pcls,uid,self.state.uid)
                            let ele = document.getElementById(pcls+'-iframe-form-input_'+uid)
                            ele.onchange = function(){
                                alert(ele.value)
                                alert(JSON.stringify(ele.files))
                                alert(JSON.stringify(ele.files[0]))
                                let fileDate = {
                                    id: uid,
                                    containerId:self.state.uid,
                                    name: ele.value,
                                    thumb : getThumb(
                                                self.props.thumb,
                                                {
                                                    type:ele.value.replace(/.+(\/[^\/]+)$/,'$1')
                                                }
                                            ) || '' , // 组件自配优化缩略图
                                    __UploadProps : {
                                        pcls: pcls ,
