...
// auto trans 'https' to 'http'  ie8,ie8-不支持
try {
	let actionErrorMsg = /^https/.test(option.action) && navigator.userAgent.toLowerCase().match(/msie ([\d.]+)/)[1] < 9 ;
	if(actionErrorMsg){
		option.action = option.action.replace(/^\s*https*:/,'http:')
	};
} catch (e) {
	console.log(e) ;
}
